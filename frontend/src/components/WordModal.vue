              ...
              <form @submit.prevent="addOrUpdateWord" class="mt-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Enter the definition (leave empty to auto-generate)"
                    rows="3"
                    class="w-full px-3 py-2 border border-primary-700 rounded-md text-sm bg-primary-800 text-primary-50 placeholder-primary-400 focus:outline-none focus:ring-1 focus:ring-secondary-500 focus:border-secondary-500 transition-colors"
                  ></textarea>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Enter an example sentence (leave empty to auto-generate)"
                    rows="3"
                    class="w-full px-3 py-2 border border-primary-700 rounded-md text-sm bg-primary-800 text-primary-50 placeholder-primary-400 focus:outline-none focus:ring-1 focus:ring-secondary-500 focus:border-secondary-500 transition-colors"
                  ></textarea>
                  <p class="text-xs text-primary-300 mt-1">
                    💡 Tip: Leave empty to auto-generate a contextual example
                  </p>
                </div>
...
async function autoGenerateDefinition() {
  if (formData.value.word.trim()) {
    ...
    try {
      ...
      const response = await chatService.explainWord(formData.value.word)
      ...
      if (!formData.value.definition.trim()) {
        formData.value.definition = parsedDefinition.definition
      }
      if (!formData.value.example.trim() && parsedDefinition.example) {
        formData.value.example = parsedDefinition.example
      }
    } catch (error) {
