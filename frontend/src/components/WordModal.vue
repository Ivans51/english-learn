...
async function autoGenerateDefinition() {
  if (formData.value.term.trim()) {
    ...
    try {
      ...
      // Handle both single values and arrays from the API response
      const meanings = Array.isArray(parsedDefinition.meanings)
        ? parsedDefinition.meanings
        : parsedDefinition.definition
        ? [parsedDefinition.definition]
        : []

      const examples = Array.isArray(parsedDefinition.examples)
        ? parsedDefinition.examples
        : parsedDefinition.example
        ? [parsedDefinition.example]
        : []

      formData.value.meaningsText = meanings.join('; ')
      formData.value.examplesText = examples.join('; ')
    } catch (error) {
