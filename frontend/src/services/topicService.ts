...
class TopicService {
  ...
  async getTopicById(id: string): Promise<Topic> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to fetch topic ${id}: ${response.status} ${response.statusText}`,
      )
    }
    return response.json()
  }
  ...
  async updateTopic(id: string, topicData: UpdateTopicRequest): Promise<Topic> {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to update topic ${id}: ${response.status} ${response.statusText}`,
      )
    }
    return response.json()
  }
  ...
  async deleteTopic(
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to delete topic ${id}: ${response.status} ${response.statusText}`,
      )
    }
    return response.json()
  }
