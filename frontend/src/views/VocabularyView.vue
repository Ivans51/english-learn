    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      <div class="flex gap-4 lg:gap-8">
        ...
        <div class="hidden lg:block w-80 flex-shrink-0">
          ...
          <div class="mb-6">
            <div class="flex items-center mb-2">
              ...
              <h1 class="text-xl sm:text-2xl font-bold text-primary-900 dark:text-primary-50">My Vocabulary</h1>
            </div>
            <p class="text-sm sm:text-base text-primary-600 dark:text-primary-400">Your personal collection of English words</p>
          </div>

          <!-- Search & Filter -->
          <div class="bg-white dark:bg-black rounded-lg p-4 mb-6 transition-colors">
            ...
            <div class="space-y-4">
              <div>
                ...
                <input type="text" v-model="searchQuery" placeholder="Search vocabulary..." class="w-full px-3 py-2 border border-primary-300 dark:border-primary-600 rounded-md text-sm bg-white dark:bg-primary-900 text-primary-900 dark:text-primary-50 placeholder-primary-400 dark:placeholder-primary-500 focus:outline-none focus:ring-1 focus:ring-secondary-500 focus:border-secondary-500 transition-colors"/>
              </div>

              <div>
                <label class="block text-sm font-medium text-primary-700 dark:text-primary-300 mb-1">Level</label>
                <select v-model="filterLevel" class="w-full px-3 py-2 border border-primary-300 dark:border-primary-800 rounded-md text-sm bg-white dark:bg-primary-900 text-primary-900 dark:text-primary-50 focus:outline-none focus:ring-1 focus:ring-secondary-500 focus:border-secondary-500 transition-colors">
              ...
              <div>
                ...
                <select v-model="selectedStatus" class="w-full px-3 py-2 border border-primary-300 dark:border-primary-600 rounded-md text-sm bg-white dark:bg-primary-900 text-primary-900 dark:text-primary-50 focus:outline-none focus:ring-1 focus:ring-secondary-500 focus:border-secondary-500 transition-colors">
          ...
          <!-- Add New Word Button -->
          <div class="space-y-3">
            <button @click="openAddWordModal" class="w-full bg-primary-900 dark:bg-primary-50 text-primary-50 dark:text-primary-950 py-2 px-3 sm:py-3 sm:px-4 rounded-md text-sm sm:font-medium hover:bg-primary-800 dark:hover:bg-primary-100 transition-colors flex items-center justify-center cursor-pointer">
        ...
        <div class="flex-1">
          ...
          <div class="lg:hidden bg-white dark:bg-black rounded-lg p-4 mb-6 transition-colors">
            <h3 class="font-medium text-primary-900 dark:text-primary-50 mb-4 flex items-center">
              <svg class="w-4 h-4 mr-2 text-primary-600 dark:text-primary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.414A1 1 0 013 6.707V4z">
                </path>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
              ...
              <div class="sm:col-span-2 lg:col-span-1">
                <button @click="openAddWordModal" class="w-full bg-primary-900 dark:bg-primary-50 text-primary-50 dark:text-primary-950 py-2 px-3 sm:py-1 sm:px-4 rounded-md text-sm font-medium hover:bg-primary-800 dark:hover:bg-primary-100 transition-colors flex items-center justify-center cursor-pointer">
                  <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6">
                    </path>
                  </svg>
          ...
          <div class="mb-4 sm:mb-6">
            <h2 class="text-base sm:text-lg font-medium text-primary-900 dark:text-primary-50 mb-1">
              {{ filteredWords.length }} words
            </h2>
          </div>

          <!-- Loading Indicator -->
          <div v-if="isLoading" class="flex flex-col sm:flex-row items-center justify-center py-8 sm:py-12 gap-3">
            <div class="animate-spin rounded-full h-10 w-10 sm:h-12 sm:w-12 border-b-2 border-primary-600 dark:border-primary-400"></div>
            <span class="text-sm sm:text-base text-primary-600 dark:text-primary-400">Loading vocabulary...</span>
          </div>
          ...
          <div v-else>
              ...
              <div class="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
                <div class="flex-1 min-w-0">
                  <div class="flex sm:items-center gap-2">
                    <h3 class="text-lg sm:text-xl font-semibold text-primary-900 dark:text-primary-50 break-words">
                      {{ word.word }}
                    </h3>
                    <div class="flex items-center gap-2">
                      <span v-if="word.status === 'mastered'" class="px-2 py-1 text-xs font-medium bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-200 rounded flex-shrink-0">
                        Mastered
                      </span>
                      <button @click="toggleWordDetails(word.id)" class="px-2 py-1 text-xs sm:text-sm font-medium text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-200 transition-colors cursor-pointer">
                        {{ expandedWords[word.id] ? 'Hide details' : 'Show details' }}
                      </button>
                    </div>
                  </div>

                  <div v-if="expandedWords[word.id]" class="transition-all duration-300">
                    <p class="text-primary-700 dark:text-primary-300 mb-3 mt-2">{{ word.definition }}</p>
                    <div class="bg-primary-50 dark:bg-primary-900 p-3 rounded italic text-primary-600 dark:text-primary-400">
                      "{{ word.example }}"
                    </div>
                  </div>
                </div>
                ...
                <div class="flex items-center gap-2 sm:gap-2">
                  <span class="text-sm text-primary-500 dark:text-primary-400">
                    ...
                    <span class="px-2 py-1 text-xs font-medium bg-primary-100 dark:bg-primary-700 text-primary-700 dark:text-primary-300 rounded">
                    {{ word.level }}
                  </span>
                  <button @click="toggleWordStatus(word)" :class="getStatusButtonClass(word)" class="text-xs sm:text-sm px-2 sm:px-3 py-1 sm:py-2 h-8 sm:h-8 flex-shrink-0">
                    {{ getStatusButtonText(word) }}
                  </button>

                  <button @click="openEditWordModal(word)" class="h-8 w-8 flex items-center justify-center text-primary-400 dark:text-primary-500 hover:text-primary-600 dark:hover:text-primary-300 rounded border border-white dark:border-primary-800 cursor-pointer flex-shrink-0">
                  ...
                  <button @click="deleteWord(word.id)" class="h-8 w-8 flex items-center justify-center text-primary-400 dark:text-primary-500 hover:text-red-600 dark:hover:text-red-400 rounded border border-white dark:border-primary-800 cursor-pointer flex-shrink-0">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Empty State -->
            <div v-if="filteredWords.length === 0 && !isLoading" class="text-center py-8 sm:py-12 px-4">
              <div class="mx-auto h-16 w-16 sm:h-24 sm:w-24 text-primary-400 dark:text-primary-500 mb-4">
                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="w-full h-full">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253">
                  </path>
                </svg>
              </div>
              <h3 class="text-base sm:text-lg font-medium text-primary-900 dark:text-primary-50 mb-2">No vocabulary words yet</h3>
              <p class="text-sm sm:text-base text-primary-500 dark:text-primary-400 mb-6 max-w-sm mx-auto">
                Start building your personal vocabulary collection by adding new words.
              </p>
              <button @click="openAddWordModal" class="inline-flex items-center px-3 py-2 sm:px-4 sm:py-2 bg-primary-900 dark:bg-primary-50 text-primary-50 dark:text-primary-950 text-xs sm:text-sm font-medium rounded-md hover:bg-primary-800 dark:hover:bg-primary-100 transition-colors cursor-pointer">
