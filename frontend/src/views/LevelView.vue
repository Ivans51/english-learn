  ...
  <div class="min-h-screen bg-primary-50 dark:bg-primary-950 transition-colors">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-8 flex justify-between items-center">
        ...
        <button
          @click="showAddTopicModal = true; resetNewTopicForm()"
      ...
      <div v-else-if="currentTopics.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
            ...
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-lg font-semibold text-primary-900 dark:text-primary-50">
                ...
                <span :class="[
                  ...
                  topic.difficulty === 'Easy' ? 'bg-green-100 dark:bg-green-800 text-green-700 dark:text-green-200' :
                  topic.difficulty === 'Medium' ? 'bg-accent-100 dark:bg-accent-800 text-accent-700 dark:text-accent-200' :
      ...
      <div v-if="showAddTopicModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div class="fixed inset-0 bg-primary-950 bg-opacity-75 transition-opacity" @click="showAddTopicModal = false; resetNewTopicForm()"></div>
        <div class="bg-white dark:bg-primary-900 rounded-lg shadow-xl overflow-hidden transform transition-all sm:max-w-lg sm:w-full p-6 relative">
          <h3 class="text-xl font-semibold text-primary-900 dark:text-primary-50 mb-6">
            {{ isEditing ? 'Edit Topic' : 'Add New Topic' }}
          </h3>
          <form @submit.prevent="saveTopic">
            <div class="space-y-4">
              <div>
                <label for="title" class="block text-sm font-medium text-primary-700 dark:text-primary-300">Title</label>
                <input
                  type="text"
                  id="title"
              ...
              <div>
                <label for="description" class="block text-sm font-medium text-primary-700 dark:text-primary-300">Description</label>
                <textarea
                  id="description"
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label for="level" class="block text-sm font-medium text-primary-700 dark:text-primary-300">Level</label>
                  <select
                    id="level"
                ...
                <div>
                  <label for="difficulty" class="block text-sm font-medium text-primary-700 dark:text-primary-300">Difficulty</label>
                  <select
                    id="difficulty"
              ...
              <div>
                <label for="duration" class="block text-sm font-medium text-primary-700 dark:text-primary-300">Duration (minutes)</label>
                <input
                  type="number"
                  id="duration"
              ...
              <div>
                <label for="tags" class="block text-sm font-medium text-primary-700 dark:text-primary-300">Tags (comma-separated)</label>
                <input
                  type="text"
                  id="tags"
            ...
            <div class="mt-6 flex justify-end space-x-3">
              <button
                type="button"
                @click="showAddTopicModal = false; resetNewTopicForm()"
...
const saveTopic = async () => {
  if (!newTopicForm.value.title.trim() || !newTopicForm.value.description.trim()) {
    toastComponent.value?.addToast({
  ...
  newTopicForm.value.tags = newTopicForm.value.tagsInput
    ? newTopicForm.value.tagsInput.split(',').map(tag => tag.trim()).filter(tag => tag.length > 0)
    : []
